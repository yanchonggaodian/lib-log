...
log_t module_log_init(const char *module, log_level_t level, int houroffset, log_outputv_callback_t log_outputv_callback, log_outputvs_callback_t log_outputvs_callback)
{
	...
	else if(timezone < -24)timezone=-24;
	if(log){
		log->module          = module;
		log->level           = level;
...
static int module_log_sprint_prefix(char* dest, log_internal_t* log, log_level_t level, 
							 const char * filename, const char* funcname, const int lineno)
{
	int prefix_len;
...
static int module_log_print_prefix(log_internal_t* log, log_level_t level, 
							 const char * filename, const char* funcname, const int lineno)
{
	int prefix_len;
	struct tm tm;
						 ...
						 const char *fmt, va_list va_l)
{
	...
	if(level <= log->level){
		int prefix_len, len;
		{
			...
			len = (prefix_len = module_log_sprint_prefix(NULL, log, level, filename, funcname, lineno)) + vsnprintf(NULL, 0, fmt, va_l);
			va_l = va_dest;
		}
		if(outaddr && len>=0){
			char * p;
			...
			prefix_len = module_log_sprint_prefix(*outaddr, log, level, filename, funcname, lineno);
						...
						const char *fmt, va_list va_l)
{
	...
	if(level <= log->level){
		int len = module_log_print_prefix(log, level, filename, funcname, lineno);
